...
#print(plotIndiv(result, comp = c(1,2),ind.names = FALSE,col = col, cex = cex[1], pch = c(2), X.label=paste("PC1 (",r1[1],"% variation)",sep=""),Y.label=paste("PC2 (",r1[2],"% variation)",sep=""),title=main_text,add.legend=TRUE,plot.ellipse=pca.ellipse,ellipse.level=ellipse.conf.level,col.per.group=col_per_group,group=samplelabels))
    ...
    plotIndiv(result, comp = c(1,2),ind.names = FALSE,col = col, cex = cex[1], pch = c(2), X.label=paste("PC1 (",r1[1],"% variation)",sep=""),Y.label=paste("PC2 (",r1[2],"% variation)",sep=""),title=main_text,size.title=rel(1),add.legend=TRUE,plot.ellipse=pca.ellipse,ellipse.level=ellipse.conf.level,col.per.group=col_per_group,group=samplelabels)
...
if(mixomics_check==1){
 ...
 main_text=paste("Pairwise PC score plots using ",filename," features after preprocessing\np-value for overall differences between groups using PC1 and PC3 in a multivariate\n one-way ANOVA model=",round(pc2_pval,3),sep="")
       ...
       plotIndiv(result, comp = c(1,3),ind.names = FALSE,col = col, cex = cex, pch = c(2), X.label=paste("PC1 (",r1[1],"% variation)",sep=""),Y.label=paste("PC3 (",r1[3],"% variation)",sep=""),title=main_text,size.title=rel(1),add.legend=TRUE,plot.ellipse=pca.ellipse,ellipse.level=ellipse.conf.level,col.per.group=col_per_group,group=samplelabels)
 ...
 plotIndiv(result, comp = c(2,3),ind.names = FALSE,col = col, cex = cex, pch = c(2), X.label=paste("PC3 (",r1[3],"% variation)",sep=""),Y.label=paste("PC2 (",r1[2],"% variation)",sep=""),title=main_text,size.title=rel(1),add.legend=TRUE,plot.ellipse=pca.ellipse,ellipse.level=ellipse.conf.level,col.per.group=col_per_group,group=samplelabels)
