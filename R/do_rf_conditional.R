...
function(X,classlabels, ntrees=500, analysismode="classification"){
	...
	if(analysismode=="classfication"){
        set.seed(290875)
	rf1<-cforest(factor(Targetvar)~.,data=dataA,control=cforest_control(teststat = "max",
   		...
   		set.seed(290875)
   		rf1<-cforest(Targetvar~.,data=dataA,control=cforest_control(teststat = "max",
                     testtype = "Teststatistic",
                     mincriterion = qnorm(0.9),
                     savesplitstats = FALSE,
                     ntree = ntrees, mtry = NULL, replace = TRUE,
    ...
    if(analysismode=="classfication"){
	set.seed(290875)
    varimp_res<-varimpAUC(rf1,conditional=TRUE) #varimpAUC(rf1,conditional=TRUE)
    }else{
