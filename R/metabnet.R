metabnet <-
function(feature_table_file,target.metab.file,sig.metab.file,class_labels_file=NA,parentoutput_dir,num_replicates=3,cor.method="spearman",abs.cor.thresh=0.4,cor.fdrthresh=0.05,
...
log2transform,medcenter,znormtransform,quantile_norm,lowess_norm,madscaling,missing.val, samplermindex,rep.max.missing.thresh,summary.na.replacement)
	...
	if(is.na(sig.metab.file)==FALSE){
						goodfeats<-read.table(sig.metab.file,sep="\t",header=TRUE,quote = "")
			}else{
...
#names(g1)
	...
	print(paste(length(unique(sigfeats.index))," significant features",sep=""))
