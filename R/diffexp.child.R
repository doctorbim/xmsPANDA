...
if(featselmethod=="rfesvm"){
    ...
    if(length(class_labels_levels)<3){
        featureRankedList = svmrfeFeatureRanking(x=t(data_m_fc),y=svm_classlabels,svmkernel=svm_kernel,kfold=kfold,pred.eval.method=pred.eval.method,analysismode=analysismode)
        
        best_subset<-featureRankedList$best_subset
        featureRankedList<-featureRankedList$featureRankedList
    }else{
        
        featureRankedList = svmrfeFeatureRankingForMulticlass(x=t(data_m_fc),y=svm_classlabels,svmkernel=svm_kernel,kfold=kfold,pred.eval.method=pred.eval.method,analysismode=analysismode)
        best_subset<-featureRankedList$best_subset
        featureRankedList<-featureRankedList$featureRankedList
    }
                ...
                for(m in 1:dim(goodfeats)[1])
                {
				    ...
				    boxplot(cur_d,ylab=ylab_text,main=mzname,xaxt="n",col=boxplot.col.opt)
                                    ...
                                    for(i in 1:length(class_labels_levels)){
                                        axis(side=1,at=c(i),labels=class_labels_levels[i], col=col_vec[i],cex.axis=cexval)
                        ...
                        axis(side=1,at=seq(1),labels=class_label_A, col="red",cex.axis=cexval)
                        axis(side=1,at=2,labels=class_label_B,col="green",cex.axis=cexval)
                    }
                }
