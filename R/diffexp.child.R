    ...
    p1<-mixOmics::pca(tempX,center=TRUE,scale=TRUE,ncomp=ncomp,multilevel=paireddesign)
                       ...
                       #regression
                            ...
                            # print(sample.col.opt)
                                                     ...
                                                     get_pca(X=data_m_fc,samplelabels=tempgroup,legendlocation=legendlocation,filename="all",ncomp=3,center=TRUE,scale=TRUE,legendcex=0.5,outloc=getwd(),col_vec=col_vec1,sample.col.opt=sample.col.opt,alphacol=0.3,class_levels=NA,pca.cex.val=pca.cex.val,pca.ellipse=FALSE,paireddesign=paireddesign)
                                ...
                                #write.table(varimp_res2, file=filename,sep="\t",row.names=FALSE)
                                                    ...
                                                    write.table(varimp_res2, file=filename,sep="\t",row.names=FALSE)
                                                                        ...
                                                                        if(is.na(max_varsel)==FALSE){
                                                                                ...
                                                                                min_thresh<-min(sorted_varimp_res[which(sorted_varimp_res>0)],na.rm=TRUE)
                                                                                ...
                                                                                sel.diffdrthresh<-row_num_vec%in%goodip #varimp_res2>=min_thresh
