...
numtrees,analysismode,net_node_colors,net_legend,svm_kernel,heatmap.col.opt,manhattanplot.col.opt,boxplot.col.opt,sample.col.opt,alphacol,pls_vip_thresh,num_nodes,max_varsel,pls_ncomp,pca.stage2.eval,scoreplot_legend,pca.global.eval,rocfeatlist,rocfeatincrement,rocclassifier,foldchangethresh,wgcnarsdthresh,WGCNAmodules,optselect,max_comp_sel,saveRda,legendlocation,degree_rank_method,pca.cex.val,pca.ellipse,ellipse.conf.level,pls.permut.count,svm.acc.tolerance,limmadecideTests,pls.vip.selection,globalclustering,plots.res,plots.width,plots.height,plots.type,output.device.type,pvalue.thresh,individualsampleplot.col.opt,pamr.threshold.select.max,mars.gcv.thresh)
{
        ...
        if(featselmethod=="rf" | featselmethod=="RF"){
            ...
            if(featselmethod=="rfconditional" | featselmethod=="RFconditional"){
			...
			#log2.fold.change.thresh_list<-c(0)
					...
					if(featselmethod=="MARS"){
						...
						print("##############Level 1: Using MARS to find discriminatory metabolites###########")
							...
							if(featselmethod=="lmreg" | featselmethod=="logitreg" | featselmethod=="poissonreg" | featselmethod=="lm1wayanova" | featselmethod=="lm2wayanova" | featselmethod=="lm1wayanovarepeat" | featselmethod=="lm2wayanovarepeat" | featselmethod=="rfesvm" | featselmethod=="wilcox" | featselmethod=="ttest" | featselmethod=="pamr" | featselmethod=="ttestrepeat"){
              ...
              print(classlabels)
				...
				if(featselmethod=="lm1wayanova" | featselmethod=="lm2wayanova" | featselmethod=="pls2way" | featselmethod=="spls2way" | featselmethod=="wilcox" | featselmethod=="ttest"){
					...
					classlabels<-read.table(class_labels_file,sep="\t",header=TRUE)
						...
						if(featselmethod=="lm1wayanovarepeat" | featselmethod=="lm2wayanovarepeat" | featselmethod=="pls1wayrepeat" | featselmethod=="spls1wayrepeat" | featselmethod=="pls2wayrepeat" | featselmethod=="spls2wayrepeat" | featselmethod=="ttestrepeat"){
							...
							pairedanalysis=TRUE
							if(featselmethod=="lm1wayanovarepeat" | featselmethod=="pls1wayrepeat" | featselmethod=="spls1wayrepeat" | featselmethod=="ttestrepeat"){
...
if(is.na(paireddesign)==TRUE){
    p1<-mixOmics::pca(tempX,center=TRUE,scale=TRUE,ncomp=ncomp)
}else{
    p1<-mixOmics::pca(tempX,center=TRUE,scale=TRUE,ncomp=ncomp,multilevel=paireddesign)
    ...
    #save(goodip,file="goodip.Rda")
                        ...
                        if(featselmethod=="RF")
                        {
                            ...
                            filename<-"RFconditional_VIM_withBoruta_allfeats.txt"
                                ...
                                temp1<-cbind(classlabels,t(data_m_fc))
                                temp1<-as.data.frame(temp1)
                                
                                set.seed(290875)
                                Boruta(classlabels~.,data=temp1,maxRuns=1000)->Bor.test
           
                                varimp_res2<-rep(0,length(Bor.test$finalDecision))
                                ...
                                #write.table(varimp_res2, file=filename,sep="\t",row.names=FALSE)
                                                    ...
                                                    write.table(varimp_res2, file=filename,sep="\t",row.names=TRUE)
						...
						barplot(sorted_varimp_res, xlab="Significant features", main=barplot_text,cex.axis=0.5,cex.names=0.4, ylab="VIM")
                    ...
                    #write.table(data_limma_fdrall_withfeats, file=filename,sep="\t",row.names=FALSE)
                        ...
                        if(output.device.type!="pdf"){
                            ...
                            temp_filename_1<-"Figures/MARS_sigfeats_GCVbarplot.png"
                            
                            png(temp_filename_1,width=plots.width,height=plots.height,res=plots.res,type=plots.type,units="in")
                        }
                        
                        
                        barplot(sorted_varimp_res, xlab="Significant features", main=barplot_text,cex.axis=0.5,cex.names=0.4, ylab="GCV")
 ...
 #r2_q2_valid_res<-rbind(plsres1$valid_res$R2,plsres1$valid_res$Q2,plsres1$valid_res$MSEP)
                    ...
                    #print(opt_comp)
									if(featselmethod=="spls"){
										...
										if(opt_comp>1){
                                            ...
                                            save(rand_pls_sel_fdr,file="rand_pls_sel_fdr.Rda")
                                            save(rand_pls_sel_prob,file="rand_pls_sel_prob.Rda")
				...
				rank_num<-match(pls_vip,ranked_vec)
                        ...
                        write.table(data_limma_fdrall_withfeats, file=filename,sep="\t",row.names=FALSE)
								...
								if(featselmethod=="lmreg" | featselmethod=="lm1wayanova" | featselmethod=="lm2wayanova" | featselmethod=="lm1wayanovarepeat" | featselmethod=="lm2wayanovarepeat"| featselmethod=="logitreg" | featselmethod=="wilcox" | featselmethod=="ttest" | featselmethod=="ttestrepeat" |  featselmethod=="poissonreg")
								{
								pvalues<-{}
...
#print(head(classlabels_response_mat))
  ...
  clusterExport(cl,"glm",envir = .GlobalEnv)
   ...
   clusterExport(cl,"anova",envir = .GlobalEnv)
											...
											#data_mat_anova<-cbind(t(data_m_fc),classlabels_response_mat)
													res1<-apply(data_m_fc,1,function(x,classlabels_response_mat,logistic_reg,poisson_reg){
...
if(is.na(zvec[1])==FALSE){
    ...
    maintext2=paste(maintext2,"\n",manhattanplot.col.opt[2],": lower in class ",class_labels_levels_main[1]," & ",manhattanplot.col.opt[1],": higher in class ",class_labels_levels_main[1],sep="")
        ...
        try(get_volcanoplots(xvec=maxfoldchange,yvec=logp,up_or_down=maxfoldchange,ythresh=ythresh,y2thresh=y2thresh,xthresh=foldchangethresh,maintext=maintext1,ylab="-log10p",xlab="log2(fold change)"),silent=TRUE)
...
if(output.device.type!="pdf"){
    ...
    dev.off()
            ...
            yincrement=0.5
            y2thresh=NA
            #save(list=c("d4","yvec_val","ythresh","zvec","x1increment","yincrement","maintext1","x2increment","maintext2","ylabel","y2thresh","data_limma_fdrall_withfeats","vip_res"),file="manhattanplot_objects.Rda")
...
try(get_manhattanplots(xvec=d4$mz,yvec=yvec_val,ythresh=pls_vip_thresh,up_or_down=zvec,xlab="mass-to-charge (m/z)",ylab="VIP",xincrement=x1increment,yincrement=0.5,maintext=maintext1,col_seq=c("black"),colorvec=manhattanplot.col.opt),silent=TRUE)
...
if(output.device.type!="pdf"){
    ...
    dev.off()
                            ...
                            try(get_manhattanplots(xvec=d4$time,yvec=yvec_val,ythresh=pls_vip_thresh,up_or_down=zvec,xlab="Retention time",ylab="VIP",xincrement=x2increment,yincrement=0.5,maintext=maintext2,col_seq=c("black"),colorvec=manhattanplot.col.opt),silent=TRUE)
...
if(length(class_labels_levels)==2){
    ...
    try(get_volcanoplots(xvec=maxfoldchange,yvec=yvec_val,up_or_down=maxfoldchange,ythresh=ythresh,xthresh=foldchangethresh,maintext=maintext1,ylab="VIP",xlab="log2(fold change)"),silent=TRUE)
...
if(output.device.type!="pdf"){
    ...
    png(temp_filename_1,width=plots.width,height=plots.height,res=plots.res,type=plots.type,units="in")
						...
						try(get_manhattanplots(xvec=d4$mz,yvec=yvec_val,ythresh=0,up_or_down=zvec,xlab="mass-to-charge (m/z)",ylab="Loading (absolute)",xincrement=x1increment,yincrement=0.1,maintext=maintext1,col_seq=c("black"),colorvec=manhattanplot.col.opt),silent=TRUE)
                            ...
                            try(get_manhattanplots(xvec=d4$time,yvec=yvec_val,ythresh=0,up_or_down=zvec,xlab="Retention time",ylab="Loading (absolute)",xincrement=x2increment,yincrement=0.1,maintext=maintext2,col_seq=c("black"),colorvec=manhattanplot.col.opt),silent=TRUE)
                                        ...
                                        if(length(class_labels_levels)==2){
                                            ...
                                            try(get_volcanoplots(xvec=maxfoldchange,yvec=yvec_val,up_or_down=maxfoldchange,ythresh=ythresh,xthresh=foldchangethresh,maintext=maintext1,ylab="(absolute) Loading",xlab="log2(fold change)",yincrement=0.1),silent=TRUE)
...
cnames1<-colnames(ord_data)
                    ...
                    if(analysismode=="classification"){
                        ...
                        if(degree_rank_method=="overall"){
                            ...
                            # save(list=ls(),file="degree_debug.Rda")
					 ...
					 if(input.intensity.scale=="log2" || log2transform==TRUE){
                        ...
                        if(logistic_reg==TRUE){
                            ...
                            if(poisson_reg==TRUE){
                                ...
                                fname4<-paste("poissonreg","results_significantfeatures.txt",sep="")
...
#write.table(data_limma_fdrall_withfeats_2,file=fname4,sep="\t",row.names=FALSE)
                    ...
                    if(logistic_reg==TRUE){
                        ...
                        fname4<-paste("logitreg_sigfeats_ordered_by_significance.txt",sep="")
                    }else{
                        
                        if(poisson_reg==TRUE){
                         
                        
                         fname4<-paste("poissonreg_sigfeats_ordered_by_significance.txt",sep="")
