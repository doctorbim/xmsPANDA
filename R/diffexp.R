...
svm_kernel="radial",heatmap.col.opt="redblue",manhattanplot.col.opt=c("darkblue","red3"),boxplot.col.opt=c("grey57"),sample.col.opt="rainbow",pls_vip_thresh=1,num_nodes=2,max_varsel=100,pls_ncomp=5,pca.stage2.eval=FALSE,scoreplot_legend=TRUE,pca.global.eval=TRUE,rocfeatlist=seq(2,11,1),rocfeatincrement=TRUE,rocclassifier="svm",foldchangethresh=2,wgcnarsdthresh=20,WGCNAmodules=TRUE,optselect=TRUE,max_comp_sel=1,saveRda=FALSE,legendlocation="topleft",pca.cex.val=4,
pca.ellipse=FALSE,ellipse.conf.level=0.95,pls.permut.count=1000,svm.acc.tolerance=5,limmadecideTests=TRUE,pls.vip.selection="max",globalclustering=TRUE,plots.res=600,plots.width=8,plots.height=8,plots.type="cairo",output.device.type="pdf",pvalue.thresh=0.05,individualsampleplot.col.opt=NA,pamr.threshold.select.max=FALSE,aggregation.method="RankAggreg",aggregation.max.iter=1000,mars.gcv.thresh=10,...)
{
...
if(consensus_analysis==TRUE){
   ...
   if(aggregation.method=="RankAggreg"){
    ...
    if(nrow(common_feats)>0){
        ...
        write.table(Xmat,file="Aggregated_significant_features.txt",sep="\t",row.names=FALSE)
...
if(analysismode=="classification")
{
        ...
        get_boxplots(X=Xmat,Y=Ymat,parentoutput_dir=outloc,sample.col.opt=sample.col.opt,boxplot.col.opt=boxplot.col.opt, alphacol=0.3,newdevice=FALSE,cex=0.6)
...
ylabel="intensity",cex.val=0.9,sample.col.opt=sample.col.opt)
 ...
 try(get_lineplots(X=Xmat,Y=classlabels_orig,feature_table_file=NA,parentoutput_dir=getwd(),class_labels_file=NA,sample.col.opt=sample.col.opt, alphacol=0.3,col_vec=col_vec,pairedanalysis=pairedanalysis,pca.cex.val=pca.cex.val,legendlocation=legendlocation,pca.ellipse=pca.ellipse,ellipse.conf.level=ellipse.conf.level,filename="significant"),silent=TRUE)
     ...
     classlabels_orig<-read.table("../Stage2/classlabels_orig.txt",sep="\t",header=TRUE,quote="",stringsAsFactors = FALSE,check.names = FALSE)
