...
#Y<-as.numeric(as.factor(Y[,1]))
    ...
    if(sparseselect==TRUE)
    {
	    ...
	    if(repeatmeasures==TRUE){
                        ...
                        linn.pls <- spls(X, Y,ncomp=opt_comp,keepX=keep_x_vec,mode="regression")
                }
	 }else{
        ...
        if(analysismode=="regression"){
            linn.pls <- pls(X, Y,ncomp=opt_comp)


        }else{
            linn.pls <- plsda(X, Yclass,ncomp=opt_comp)
        }
   ...
   # SS<-c(linn.pls$loadings$X)^2*colSums(linn.pls$variates$X^2)
    ...
    #print(opt_comp)
        ...
        #cex <- rep(0.7,length(col.stimu))
        #pch.time<-rep(15,length(col.stimu))
        
        #  print("col")
        # print(col.stimu)
        # print("pch time")
        # print(pch.time)
        #print(class_labels_levels2)
        
        #l1<-class_labels_levels
       
	#plotIndiv(linn.pls, col = col.stimu) 
    #cex <- rep(2, length(col.stimu))
    ...
    if(output.device.type!="pdf"){
        ...
        temp_filename_1<-"Figures/PLS_pairwise_component_plots.png"
        
        png(temp_filename_1,width=plots.width,height=plots.height,res=plots.res,type=plots.type,units="in")
        }
    
    try(get_plsplots(X,plsres=linn.pls,plsvar=pls_var,samplelabels=Yclass,filename=NA,ncomp=opt_comp,center=TRUE,scale=TRUE,legendcex=0.5,outloc=getwd(),col_vec=col.stimu,sample.col.opt="default",alphacol=0.3,legendlocation="topright",class_levels=class_labels_levels),silent=TRUE)
