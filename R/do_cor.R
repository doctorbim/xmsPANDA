...
#dev.off()
    ...
    #tiff("network_mzlabels.tiff",res=300)
...
#tiff(pdfname,res=300)
	...
	net_fname<-paste("network_maxcor",max.cor.num,"_mzlabels.tiff",sep="")
    #tiff(net_fname, res=300,compression="lzw") #width=plots.width,height=plots.height,res=plots.res, compression="lzw")
	set.seed(netrandseed)
	net_result<-try(network(mat=as.matrix(nonsig_vs_fdr0.05_pearson_mat_bool_filt_1), threshold=abs.cor.thresh,color.node = net_node_colors,shape.node = c("rectangle", "circle"),cex.node.name=0.4,
	color.edge = c("red", "blue"), lwd.edge = 1,show.edge.labels = FALSE, interactive = FALSE,cex.node.name=0.6),silent=TRUE)
