get_barplots <-
function(feature_table_file,class_labels_file,X=NA,Y=NA,parentoutput_dir,newdevice=FALSE,ylabel="Intensity",bar.colors=NA,cex.val=0.6,barplot.col.opt=c("grey57")){
    ...
    lapply(2:dim(mtcars)[2],function(i){
        ...
        max_yval1<-max(max_yval,max(limits1,na.rm=TRUE)+0.5,na.rm=TRUE)
        
        max_yval<-max(max_yval,max_yval1,na.rm=TRUE)
        
        min_yval1<-min(min_yval,(min(limits1,na.rm=TRUE)-0.5),na.rm=TRUE)
        min_yval1<-max(0,min_yval1,na.rm=TRUE)
        ...
        geom_errorbar(limits, position = dodge, width = 0.3,size=0.5) + coord_cartesian(ylim=c(min_yval1,max_yval1)) +scale_y_continuous(limits=c(0,max_yval)) + theme(text = element_text(size=9))
