...
function(xvec,yvec,up_or_down,maintext="",ythresh=0.05,y2thresh=NA,ylab,xlab,colorvec=c("darkblue","red3"),col_seq=c("brown","chocolate3","orange3","coral","pink","skyblue","blue","darkblue","purple","violet"),xincrement=1,yincrement=1,xthresh=1,pchvec=c(21,21),background.points.col="gray50",bad.feature.index=NA){
   ...
   if(length(bad.feature.index)>0){
    ...
    for(i in goodip){
        if(up_or_down[i]>0){
            points(d4[i],logp[i],col=colorvec[1],cex=0.7,pch=pchvec[1],bg=colorvec[1]); points(d4[i],logp[i],col=colorvec[1],cex=0.4,bg=colorvec[1])
        }else{
            
            points(d4[i],logp[i],col=colorvec[2],cex=0.7,pch=pchvec[2],bg=colorvec[2]); points(d4[i],logp[i],col=colorvec[2],cex=0.4,bg=colorvec[2])
        }
    }
    if(length(bad.feature.index)>0){
